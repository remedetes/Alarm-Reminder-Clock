...
if(isset($_GET['id'])){
    ...
    if(mysql_num_rows($query) > 0){
        ...
        $sql = "UPDATE `komentar` SET `status`=1 WHERE `id_komentar`='$id'";
        $query = mysql_query($sql);
    }else{
        echo "Not Found";
        die();
...
if($_POST){
    ...
    $sql = "INSERT INTO `komentar`(`id_berita`,`nama`,`isi_komentar`,`status`,`hak_akses`,`tanggal`) VALUES ('$id_berita','$nama','$balas',1,1,'$tanggal')";
    if (mysql_query($sql)) {
        echo"<script>alert('Komentar Berhasil di Tambah');document.location.href='komentar.php';</script>";
    } else {
  ...
  <div class="wrap">
    ...
    <div class="blog">
      <div class="conteudo">
        ...
        <h1>Lihat Komentar</h1>
        <hr>
        <p>
          <!-- DATABASE -->
            <form action="lihat.php?id=<?php echo $_GET['id']; ?>" method="POST">
            <table width="90%" align="center">
              ...
              <tr>
                <td>Tanggal</td>
                <td>:</td>
                <?php echo "<td>".date('d-M-Y H:i:s',strtotime($tanggal))."</td>"; ?>
              </tr>
              <tr>
                <td width="20%" height="50">Isi Komentar</td>
                <td>:</td>
                <td><?php echo $isi_komentar;?></td>
              </tr>
              <tr>
                <td height="50">Balas Komentar</td>
                <td >:</td>
                <td >
                  <textarea placeholder="Balas Komentar" name="balas" id="balas" style="width: 500px;height: 150px;"></textarea>
                </td>
              </tr>              
              <tr>
                <td colspan="3" height="50" align="right">
                  <input type="submit" name="button" id="button" value="Tambahkan" />
                </td>
              </tr>
            </table>
            </form>
